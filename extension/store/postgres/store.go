// Package postgres implements the Trove store interface using grove ORM (PostgreSQL).
package postgres
...
func (s *Store) Migrate(ctx context.Context, extraGroups ...*migrate.Group) error {
	...
	if err != nil {
		return fmt.Errorf("trove/postgres: create migration executor: %w", err)
	}
	...
	if _, err := orch.Migrate(ctx); err != nil {
		return fmt.Errorf("trove/postgres: migration failed: %w", err)
	}
...
func (s *Store) CreateBucket(ctx context.Context, b *model.Bucket) error {
	...
	b.CreatedAt = now
	b.UpdatedAt = now
	_, err := s.pg.NewInsert(b).Exec(ctx)
	return err
}

func (s *Store) GetBucket(ctx context.Context, id string) (*model.Bucket, error) {
	b := new(model.Bucket)
	if err := s.pg.NewSelect(b).Where("id = ?", id).Scan(ctx); err != nil {
...
func (s *Store) GetBucketByName(ctx context.Context, name string) (*model.Bucket, error) {
	b := new(model.Bucket)
	if err := s.pg.NewSelect(b).Where("name = ?", name).Limit(1).Scan(ctx); err != nil {
...
func (s *Store) UpdateBucket(ctx context.Context, b *model.Bucket) error {
	...
	_, err := s.pg.NewUpdate(b).WherePK().Exec(ctx)
	return err
}

func (s *Store) DeleteBucket(ctx context.Context, id string) error {
	_, err := s.pg.NewDelete(&model.Bucket{}).Where("id = ?", id).Exec(ctx)
	return err
}
...
func (s *Store) CreateObject(ctx context.Context, o *model.Object) error {
	...
	o.CreatedAt = now
	o.UpdatedAt = now
	_, err := s.pg.NewInsert(o).Exec(ctx)
	return err
}

func (s *Store) GetObject(ctx context.Context, id string) (*model.Object, error) {
	o := new(model.Object)
	if err := s.pg.NewSelect(o).Where("id = ?", id).Scan(ctx); err != nil {
...
func (s *Store) UpdateObject(ctx context.Context, o *model.Object) error {
	...
	_, err := s.pg.NewUpdate(o).WherePK().Exec(ctx)
	return err
}

func (s *Store) SoftDeleteObject(ctx context.Context, id string) error {
	now := time.Now().UTC()
	_, err := s.pg.NewUpdate(&model.Object{}).
		Where("id = ?", id).
		Set("deleted_at = ?", now).
		Set("updated_at = ?", now).
		Exec(ctx)
...
func (s *Store) UpdateUploadSession(ctx context.Context, u *model.UploadSession) error {
	...
	_, err := s.pg.NewUpdate(u).WherePK().Exec(ctx)
	return err
}

func (s *Store) ListUploads(ctx context.Context) ([]*model.UploadSession, error) {
	var uploads []*model.UploadSession
	if err := s.pg.NewSelect(&uploads).OrderExpr("created_at DESC").Scan(ctx); err != nil {
...
func (s *Store) GetCASEntry(ctx context.Context, hash string) (*model.CASEntry, error) {
	...
	if err := s.pg.NewSelect(e).Where("hash = ?", hash).Scan(ctx); err != nil {
...
func (s *Store) ListCASEntries(ctx context.Context) ([]*model.CASEntry, error) {
	...
	if err := s.pg.NewSelect(&entries).OrderExpr("created_at DESC").Scan(ctx); err != nil {
...
func (s *Store) IncrementCASRef(ctx context.Context, hash string) error {
	_, err := s.pg.NewUpdate(&model.CASEntry{}).
		Where("hash = ?", hash).
...
func (s *Store) DeleteCASEntry(ctx context.Context, hash string) error {
	_, err := s.pg.NewDelete(&model.CASEntry{}).Where("hash = ?", hash).Exec(ctx)
	return err
}
...
func (s *Store) ListQuotas(ctx context.Context) ([]*model.Quota, error) {
	...
	if err := s.pg.NewSelect(&quotas).OrderExpr("updated_at DESC").Scan(ctx); err != nil {
...
func (s *Store) GetQuota(ctx context.Context, tenantKey string) (*model.Quota, error) {
	q := new(model.Quota)
	if err := s.pg.NewSelect(q).Where("tenant_key = ?", tenantKey).Scan(ctx); err != nil {
...
func (s *Store) DeleteQuota(ctx context.Context, tenantKey string) error {
	_, err := s.pg.NewDelete(&model.Quota{}).Where("tenant_key = ?", tenantKey).Exec(ctx)
	return err
}
